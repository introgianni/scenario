scenarioApp.controller('AccessController', function AccessController($scope,
    dataRetriever, ateneiService, parconfService) {
    ...
    var dataPostSuccessCallback = function(res) {
        ...
        $scope.node = JSON.parse(res.data.rawJson);
        $scope.breadcrumb.push($scope.node);
    };
    ...
    $scope.next = function(node, id) {
        var numFigli = node.children.length;
        for (var index = 0; index < numFigli; index++) {
            if (node.children[index].id === id) {
                $scope.node = node.children[index];
                $scope.breadcrumb.push($scope.node);
                return;
            }
        }
    ...
    $scope.breadcrumbNav = function(bread) {
        var lunghezzaBc = $scope.breadcrumb.length;
        for (var index = lunghezzaBc - 1; index >= 0; index--) {
            if (!(bread.id === $scope.breadcrumb[index].id)) {
